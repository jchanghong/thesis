...
\input{codetex/NettyServer.kt.tex}
...
\input{codetex/CommandPacket.kt.tex}

协议主要有2个阶段，一个是认证阶段，一个是命令阶段，认证阶段就是
接受客服端的请求，然后检查用户的认证信息，比如用户名和密码，
下面代码主要用来实现用户认证的功能。

\input{codetex/MysqlAuthHander.kt.tex}

认证成功以后就是命令阶段，服务器接受客服端的命令，然后处理，
下面是命令处理的具体代码。

\input{codetex/MysqlCommandHandler.kt.tex}
\subsection{SQL解析模块}
\pictable[htbp]{SQL前端连接模块相关的类}{}{codepdf/jsqlsql}
sql解析主要用到了druid开源的框架，解析sql以后就要做具体的数据处理，
下面的代码主要用处理用户的sql语句。

\input{codetex/MysqlSQLhander.kt.tex}

\input{codetex/SqlStatementHander.kt.tex}
...
\input{codetex/HtreeNode.java.tex}

\input{codetex/HTreeMap.java.tex}

除了实现数据结构以外还要实现存储引擎，也就是把数据存储在文件系统当中，
其中主要用到内存映射文件，下面是其中的关键代码。

\input{codetex/MStorage.java.tex}

\input{codetex/DiscIO.java.tex}
